...
export function buildApp(options = {}) {
  ...
  app.get('/api/knowledge/:slug', {
    schema: {
      params: {
        ...
        required: ['slug'],
        properties: { slug: { type: 'string', minLength: 1, pattern: '^[a-z0-9]+(?:-[a-z0-9]+)*$' } }
      }
    }
  }, async (request, reply) => {
  ...
  app.setErrorHandler((error, request, reply) => {
    if (error.validation) {
      return reply.code(400).send({ error: { code: 'VALIDATION_ERROR', message: 'Request validation failed', requestId: request.id } });
    }
    request.log.error(error);
    return reply.code(error.statusCode >= 400 && error.statusCode < 500 ? error.statusCode : 500).send({
