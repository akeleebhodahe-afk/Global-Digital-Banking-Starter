...
function apiCard(item, action = '') {
  return `<article class="feature-card"><span class="card-icon">▤</span><h3>${escapeHtml(item.title || item.name)}</h3><p>${escapeHtml(item.excerpt || item.description || '')}</p>${action}</article>`;
}
...
async function syncKnowledge() {
  ...
  try {
    ...
    grid.innerHTML = result.data.map(item => apiCard(item, `<a href="/resources" data-route>Explore resources →</a>`)).join('');
    clearApiStatus();
  } catch (error) {
    setApiStatus(`Knowledge is currently using the local preview. ${error.message}`, 'warning');
  }
}
...
async function syncResources() {
  ...
  try {
    const result = await getJson('/api/resources');
    list.innerHTML = result.data.map(item => `<a href="${escapeHtml(item.href)}" data-route><span>${escapeHtml(item.type)}</span><strong>${escapeHtml(item.title)}</strong><em>Explore →</em></a>`).join('');
    clearApiStatus();
  } catch (error) {
    setApiStatus(`Resources are currently using the local preview. ${error.message}`, 'warning');
  }
}
...
async function syncDashboard() {
  ...
  try {
    const [profile, accounts, transfers] = await Promise.all([
      getJson('/api/demo/profile'), getJson('/api/demo/accounts'), getJson('/api/demo/transfers')
    ]);
    ...
    setApiStatus(`Dashboard is currently using the local preview. ${error.message}`, 'warning');
  }
}

function syncPageFromApi() {
  const path = window.location.pathname;
  if (path === '/knowledge') syncKnowledge();
  if (path === '/resources') syncResources();
  if (path === '/membership') syncMembership();
  if (path === '/dashboard') syncDashboard();
}
